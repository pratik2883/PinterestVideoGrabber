...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/extract', async (req, res) => {
    try {
      ...
      if (!pinIdMatch) {
        return res.status(400).json({ message: "Could not extract pin ID from URL" });
      }
      ...
      if (axios.isAxiosError(error)) {
        if (error.code === 'ECONNABORTED') {
          return res.status(504).json({ message: "Connection timeout. Pinterest might be slow or blocking our request." });
        }
        
        if (error.response) {
          const status = error.response.status;
          if (status === 404) {
            return res.status(404).json({ message: "Pinterest pin not found. The pin might have been deleted or is private." });
          }
